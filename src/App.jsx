...
const CITY_ALIASES = [
  ...
  "brac",
  "korcula",
  "beograd", // ✅ dodano (treba ti)
];

/**
 * ✅ PATCH: prepoznavanje padeža i varijanti ("karlovcu", "zagrebu", "beogradu"...)
 * - minimalno, bez refaktora: samo dodatni fallback u detectCity()
 */
function detectCity(text) {
  ...
  // ✅ PATCH: padeži / oblici (najčešći)
  if (t.includes("zagrebu") || t.includes("zagrebom") || t.includes("zagreba")) return "Zagreb";
  if (t.includes("karlovcu") || t.includes("karlovca") || t.includes("karlovcem") || t.includes("karlovc")) return "Karlovac";
  if (t.includes("beogradu") || t.includes("beogradom") || t.includes("beograda")) return "Beograd";
  if (t.includes("splitu") || t.includes("splita")) return "Split";
  if (t.includes("rijeci") || t.includes("rijeke")) return "Rijeka";
  if (t.includes("osijeku") || t.includes("osijeka")) return "Osijek";
  if (t.includes("zadru") || t.includes("zadra")) return "Zadar";
  if (t.includes("puli") || t.includes("pule")) return "Pula";
  if (t.includes("dubrovniku") || t.includes("dubrovnika")) return "Dubrovnik";
...
/**
 * ✅ PATCH: “smeštaj” -> normalize() => "smestaj" (nije se hvatalo)
 * Dodajem samo "smestaj" u intent, ništa drugo ne diram.
 */
...
export default function App() {
  ...
  const computeConciergeRecs = (city) => {
    ...
    return [
      { title: "Family-friendly hotel (24h recepcija, parking)", desc: "Kasni dolazak + obiteljska sigurnost, predvidljiva usluga." },
      { title: "Mirniji boutique smještaj (tiho, centar, dobro osvijetljeno)", desc: "Manje stresa nakon puta; lakše pješke po centru." },
      { title: "Apartman s dobrom izolacijom (self check-in)", desc: "Fleksibilno vrijeme dolaska; privatnost i mir." },
    ].map((x) => ({ ...x, city: c }));
  };
  ...
  const handleUserText = async (raw, { fromVoice = false } = {}) => {
    ...
    if (intent.accommodation) {
      ...
      // ✅ razgovor nastavlja (ostaje tvoj flow)
