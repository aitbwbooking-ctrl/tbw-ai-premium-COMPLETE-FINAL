 ...
 * -------------------------------------------------------------------
 * GOALS (per your locked rules):
 * - NO "short" rewrite. This is a full single-file App.jsx you paste.
 * - Fixed header + ticker + hero + main AI search. Scroll only below search area.
 * - Mic = continuous conversation (no SEND needed). SEND = typing only.
 ...
 * - Never treats "hotelu/apartmanu/smještaju..." as city.
 * - Booking runs INSIDE TBW modal (iframe) so you can EXIT and continue talking.
 * - Handles mobile mic re-activation (browser limitation) + de-dup speech repeats.
 * - Includes locked flow: Intro -> Terms -> Robot checkbox -> Permissions gate.
 * - Tier system scaffold (TRIAL/DEMO/PREMIUM) with device/IP-like storage enforcement.
 *
 * NOTE:
 * - Web Speech API works best on Chrome/Edge. iOS Safari is limited.
 * - For true background navigation/overlay & push alarms, that's separate (PWA/SW/native),
 *   but this file includes consent + Notification API hook points and UI.
 */
...
const TBW_TIER_KEY = "tbw_tier_state_v1";
const TBW_DEVICE_KEY = "tbw_device_id_v1";
...
function initTierState() {
  ...
  const state = {
    ...
    purchasedAt: null,
    // demo limits would live here if needed
  };
...
function detectCity(text) {
  ...
  for (const re of patterns) {
    ...
    if (m?.[1]) {
      let chunk = m[1];
      // cut on known separators
      chunk = chunk
        .split(/\b(osob|osobe|odrasl|djece|djeca|od|do|datumi|dates|checkin|checkout|za)\b/i)[0]
        .trim();
  ...
  // "booking Paris" without preposition
  const withoutKw = cleaned
    .replace(/\b(smjestaj|smještaj|booking|airbnb|hotel|hotelu|apartman|apartmanu|rezervacija|rezerviraj|book|accommodation|stay|room|rooms)\b/gi, " ")
  ...
  // if the whole input itself looks like a city (1-4 words) and not an intent sentence
  const whole = cleaned;
  const wholeParts = whole.split(" ").filter(Boolean);
...
function Ticker({ text, tone = "warn" }) {
  const dot =
    tone === "danger" ? "rgba(231, 76, 60, 0.9)" : tone === "good" ? "rgba(46,204,113,0.9)" : "rgba(241,196,15,0.9)";
  const bg =
    tone === "danger"
      ? "rgba(231, 76, 60, 0.10)"
      : tone === "good"
