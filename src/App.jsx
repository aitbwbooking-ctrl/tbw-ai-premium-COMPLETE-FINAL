...
// ✅ DODANO: beograd (+ par čestih regionalnih opcija možeš kasnije širiti)
const CITY_ALIASES = [
  "zagreb",
  "beograd",
  "novi sad",
...
// ✅ DEDUPE: makni uzastopne duple riječi ("beograd beograd beograd" -> "beograd")
...
function detectCity(text) {
  ...
  // patterns like "u zagrebu" / "u beogradu"
...
export default function App() {
  ...
  // ✅ DEDUPE GUARD za voice (sprječava “isti final opet”)
  ...
  // ✅ TTS voice selector (ako postoji “humaniji” glas)
  const pickVoice = (lang = "hr") => {
    try {
      const voices = window.speechSynthesis?.getVoices?.() || [];
      if (!voices.length) return null;

      const wanted = voices.filter((v) => (v.lang || "").toLowerCase().startsWith(lang));
      const preferLocal = wanted.find((v) => v.localService && !/android/i.test(v.name || ""));
      if (preferLocal) return preferLocal;

      const notAndroid = wanted.find((v) => !/android/i.test(v.name || ""));
      return notAndroid || wanted[0] || voices[0] || null;
    } catch {
      return null;
    }
  };
  ...
  const handleUserText = async (raw, { fromVoice = false } = {}) => {
    ...
    // ✅ de-dupe voice (ukloni ponavljanja riječi iz SR)
    ...
    if (intent.accommodation) {
      ...
      // ✅ govor samo ako je TTS dostupan (i korisnik već kliknuo mic/something)
      await speak(follow);
      return;
    }
    ...
    if (!recogRef.current) {
      ...
      r.onresult = (event) => {
        ...
        if (final) {
          ...
          const preview = collapseRepeats(finalBuffer.trim());
          ...
          restartTimerRef.current = setTimeout(() => {
            ...
            // ✅ ključ: ignoriraj identičan final u kratkom vremenu
            const now = Date.now();
      ...
      r.onerror = (e) => {
        if (e?.error === "no-speech") {
          ...
          setStatus("⚠️ Voice: not-allowed (provjeri dopuštenje mikrofona u Chromeu)");
  ...
  useEffect(() => {
    ...
    // ✅ “zagrij” voice listu (pomaže da pickVoice radi odmah)
    try {
      if (canTTS) window.speechSynthesis.getVoices();
    } catch {}
  }, []);
  ...
  const heroStyle = {
    ...
    borderRadius: 22,
    border: "1px solid rgba(255,255,255,0.10)",
    background: "linear-gradient(180deg, rgba(255,255,255,0.06), rgba(2
